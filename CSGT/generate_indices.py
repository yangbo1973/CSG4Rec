...
def parse_args():
    ...
    parser.add_argument('--is_user', action='store_true', help='is user or item')
...
output_file = f"{dataset}.index.epoch{args_setting.epoch}.alpha{args_setting.alpha}-beta{args_setting.beta}-{timestamp}.json"
output_file = os.path.join(output_dir,output_file)
device = torch.device(args_setting.device if torch.cuda.is_available() else "cpu")
ckpt = torch.load(ckpt_path,  weights_only=False,map_location=torch.device('cpu'))
args = ckpt["args"]
state_dict = ckpt["state_dict"]
print(args)

data = ItemDataset(args.data_path,mode= 'test')
...
if args_setting.is_user:
    prefix = ["<ua_{}>","<ub_{}>","<uc_{}>","<ud_{}>","<ue_{}>","<uf_{}>","<ug_{}>","<uh_{}>"]
else:
    prefix = ["<ia_{}>","<ib_{}>","<ic_{}>","<id_{}>","<ie_{}>","<if_{}>","<ig_{}>","<ih_{}>"]
...
for dt in tqdm(data_loader):
    batch_items = []
    for i in range(len(dt['text_emb'])):
        batch_items.append({
            "id": int(dt['id'][i].item()),
            "text_emb": dt['text_emb'][i].to(device),
        })
    d = torch.stack([bi['text_emb'] for bi in batch_items])  # B x D_text
    d = d.to(device)
...
while True:
    ...
    for collision_items in collision_item_groups:
        d = data.text_emb[collision_items].to(device)
