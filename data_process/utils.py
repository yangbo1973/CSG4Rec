...
def load_plm(plm_name_or_path, device="cuda", use_8bit=False):
    """
    通用预训练模型加载函数
    支持 BERT / RoBERTa / LLaMA-2 系列
    """

    print(f"Loading PLM: {plm_name_or_path}")

    # 1. 加载 tokenizer
    tokenizer = AutoTokenizer.from_pretrained(plm_name_or_path)

    # 2. 特殊处理 LLaMA 的 pad token
    if tokenizer.pad_token_id is None:
        tokenizer.pad_token = tokenizer.eos_token
        print(f"Pad token not found. Set pad_token = eos_token ({tokenizer.pad_token})")

    # 3. 加载模型
    if "llama" in plm_name_or_path.lower():
        # 对 LLaMA 建议用 LlamaModel (只要 encoder 部分，不要 LM head)
        if use_8bit:
            ...
            model = LlamaModel.from_pretrained(plm_name_or_path).to(device)
    else:
        # BERT 类模型
        if use_8bit:
            ...
            model = AutoModel.from_pretrained(plm_name_or_path).to(device)
