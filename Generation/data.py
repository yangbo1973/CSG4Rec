...
class BaseDataset(Dataset):
    ...
    def _load_data(self):
        ...
        with open(os.path.join(self.data_path, self.dataset + self.user_index_file), 'r') as f:
            self.indices_user = json.load(f)
    ...
    def get_new_tokens(self):
        ...
        for index in self.indices_user.values():
            for token in index:
                self.new_tokens.add(token)
        if len(self.sep_token) > 0:
            for token in self.sep_token:
                self.new_tokens.add(token)
    ...
    def _process_train_data(self):
        ...
        for uid  in self.remapped_inters:
            ...
            for i in range(1, len(items)):
                ...
                if self.sep_token is not None and len(self.sep_token) > 0:
                    uid_tokens = ["".join(self.indices_user[str(uid)])]
                    history = (
                        [self.sep_token[0]] + uid_tokens + [self.sep_token[1]]  # <USR_GLOBAL> user </USR_GLOBAL>
                        + [self.sep_token[2]] + history + [self.sep_token[3]]   # <HIST> history </HIST>
                    )
                else:
                    history = ["".join(self.indices_user[str(uid)])] +history
